...
export default function MainLayout(props: Props) {
		...
		try {
			...
			if (res.ok) {
				...
				if (res.message === "ok") {
					...
					toast.success("Admin-Modus aktiviert")
				} else if (res.message === "fail") toast.error("falsches Passwort")
	...
	return (
		<>
			...
			/>
			<Box className={classes.mainBox} width={props.width} height={props.screenDimensions.height - 1}>
				<Box className={classes.blockBox}>
					...
					/>
					<Box className={classes.hCenter} marginTop={`${props.screenDimensions.height - 370}px`} marginBottom={2}>
						<IconButton icon="person" disableEffects height={40} width={40} scale={1.5} selected={props.permission === "user"} theme="white" onClick={() => props.setPermission("user")} tooltip="Bediener" />
						<IconButton
							icon="admin"
							disableEffects
							height={40}
							...
							theme="white"
							onClick={() => {
								if (props.permission !== "admin") setShowPassModal(true)
							}}
							tooltip="Admin"
						/>
					</Box>
					...
					/>
					<Box className={classes.hCenter} marginTop={1}>
						<Text type="p1" text={currentVersion} color="#fff" />
					</Box>
				</Box>
			</Box>
