...
function Report3(props: Props) {
	...
	const getAvg = (m: Measurement) => {
		if (props.setup.is_half_plate) {
			return avg([toSq(Number(m.l_res1.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res2.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res5.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res6.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res9.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res10.resistance), m.constants.sample_width, m.constants.spot_electrode_gap)]).toFixed(2)
		}
		return avg([toSq(Number(m.l_res1.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res2.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res3.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res4.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res5.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res6.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res7.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res8.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res9.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res10.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res11.resistance), m.constants.sample_width, m.constants.spot_electrode_gap), toSq(Number(m.l_res12.resistance), m.constants.sample_width, m.constants.spot_electrode_gap)]).toFixed(2)
	}
				...
				return (
					<td
						style={{
							...
							borderBottom: "1px solid #eee",
							fontStyle: Number(value) > Number(props.setup.max_tres) || Number(value) < Number(props.setup.min_tres) ? "italic" : undefined,
			...
			case "avg":
				return (
					...
					>
						...
						<span
							style={{
								fontStyle: Number(value) > Number(props.setup.max_lres) / 1000 || Number(value) < Number(props.setup.min_lres) / 1000 ? "italic" : undefined,
			...
			case "l_res":
				return (
					<td
						style={{
							width: "20%",
							fontStyle: (Number(value) > Number(props.setup.max_lres) / 1000 || Number(value) < Number(props.setup.min_lres) / 1000) && !hide ? "italic" : undefined,
				...
				>
					<tbody>
						...
						<tr>
							...
							<td style={{ width: "50%", fontWeight: 600 }}>{Number(props.setup.target_thickness).toFixed(3)}</td>
						</tr>
						<tr>
							<td style={{ width: "50%" }}>IST-Plattendicke [mm]</td>
							<td style={{ width: "50%", fontWeight: 600 }}>{Number(props.order.thickness).toFixed(3)}</td>
						</tr>
						<tr>
	...
	const footer = (
		...
		>
			<Box style={{ width: "30%" }}>
				<Typography>Messdatum: {getDate()}</Typography>
			</Box>
			<Box style={{ width: "45%" }}>
				<Typography>Kontrolleur: {props.responsible}</Typography>
			</Box>
			<Box style={{ width: "25%" }}>
