				...
				>
					<tbody>
						...
						<tr>
							...
							<td style={{ width: "60%", fontWeight: 600 }}>{Number(props.setup.target_thickness).toFixed(3)}</td>
						</tr>
						<tr>
							<td style={{ width: "40%" }}>IST-Plattendicke [mm]</td>
							<td style={{ width: "60%", fontWeight: 600 }}>{Number(props.order.thickness).toFixed(3)}</td>
						</tr>
						<tr>
							<td style={{ width: "60%" }}>Elektrodenabstand [mm] </td>
							<td style={{ width: "40%", fontWeight: 600 }}>{Number(props.setup.is_half_plate ? props.setup.electrode_half_distance : props.setup.electrode_distance).toFixed(0)}</td>
						</tr>
						<tr>
				...
				.map((m, i) =>
					i >= from - 1 && i <= to - 1 ? (
						<tr key={i} style={{ textAlign: "center", fontSize: 14 }}>
							...
							{validateValue("w_res", (Number(m.w_res.resistance) / 1000) * (m.constants.sample_width / (props.setup.is_half_plate ? m.constants.electrode_half_distance : m.constants.electrode_distance)))}
