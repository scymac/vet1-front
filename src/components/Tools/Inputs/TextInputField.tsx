...
export default function TextInputField(props:Props) {
  ...
  useEffect(() => {
    if (props.gainFocus !== undefined) { if (props.gainFocus === props.ndx) document.getElementById('TextInputField')!.focus() }
  }, [props.gainFocus])
  ...
  const onKeyDown = (event:any) => {
    setValue(event.target.defaultValue)
    if (props.onKeyDown !== undefined) props.onKeyDown({ keyCode: event.keyCode, text: event.target.defaultValue })
    if (event.keyCode === 13 && props.blurOnEnter) document.getElementById('TextInputField')!.blur()
  }
