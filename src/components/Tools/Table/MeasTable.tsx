...
export default function MeasTable(props: Props) {
      ...
      if (sortDir) {
        ...
        setTableData(
          sortData.sort((a, b) => {
            ...
            return (
              Date.parse(`${aa[1]}.${aa[0]}.${aa[2]}`) -
              Date.parse(`${bb[1]}.${bb[0]}.${bb[2]}`)
            )
          })
        )
      } else {
      ...
      if (sortDir) {
        ...
        setTableData(
          sortData.sort((a, b) =>
            avg(a.slice(6))
              .toString()
              .localeCompare(avg(b.slice(6)).toString(), undefined, {
                numeric: true
              })
          )
        )
        ...
        setTableData(
          sortData.sort((a, b) =>
            avg(b.slice(6))
              .toString()
              .localeCompare(avg(a.slice(6)).toString(), undefined, {
                numeric: true
              })
          )
        )
  ...
  const getCellColor = (
    value: string,
    spec: 'lRes' | 'wRes' | 'tRes' | 'tick'
  ) => {
    if (spec === 'lRes' && props.showSResOutOfSpec) {
      return Number(value) > props.setup.lRes_max / 1000 ||
        Number(value) < props.setup.lRes_min / 1000
        ? themeColors.error.main
        : themeColors.gray.dark
    }
    if (spec === 'wRes' && props.showSResOutOfSpec) {
      return Number(value) > props.setup.wRes_max / 1000 ||
        Number(value) < props.setup.wRes_min / 1000
        ? themeColors.error.main
        : themeColors.gray.dark
    }
    if (spec === 'tRes' && !props.showTResOutOfSpec) {
      return Number(value) > props.setup.tRes_max / 1000 ||
        Number(value) < props.setup.tRes_min / 1000
        ? themeColors.error.main
        : themeColors.gray.dark
    }
  ...
  const getTooltip = (type: string, d: string[][], r: number) => {
    try {
      switch (type) {
        case 'tres_a':
          return props.rawData.filter(rd => rd.sample_no === Number(d[r][0]))[0]
            .t_res.current
        case 'tres_v':
          return props.rawData.filter(rd => rd.sample_no === Number(d[r][0]))[0]
            .t_res.voltage
  ...
  const getLResTooltip = (d: string[][], r: number) => {
    try {
      const row = props.rawData.filter(
        rd => rd.sample_no === Number(d[r][0])
      )[0]
      return (
    ...
    if (c === 1) {
      return (
        <Tooltip
          arrow
          title={
            <div style={{ fontSize: 13 }}>{d[r][c] ? 'fertig' : 'offen'}</div>
          }
              ...
              <div className={classes.rTableDiv}>
                ...
                >
                  <tbody>
                    <tr>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][6], 'lRes') }}>
                          {d[r][6]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][7], 'lRes') }}>
                          {d[r][7]}
                        </Box>
                      </td>
                      {}
                      <td>
                        <Box sx={{ color: getCellColor(d[r][8], 'lRes') }}>
                          {d[r][8]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][9], 'lRes') }}>
                          {d[r][9]}
                        </Box>
                      </td>
                    </tr>
                    <tr>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][10], 'lRes') }}>
                          {d[r][10]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][11], 'lRes') }}>
                          {d[r][11]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][12], 'lRes') }}>
                          {d[r][12]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][13], 'lRes') }}>
                          {d[r][13]}
                        </Box>
                      </td>
                    </tr>
                    <tr>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][14], 'lRes') }}>
                          {d[r][14]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][15], 'lRes') }}>
                          {d[r][15]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][16], 'lRes') }}>
                          {d[r][16]}
                        </Box>
                      </td>
                      <td>
                        <Box sx={{ color: getCellColor(d[r][17], 'lRes') }}>
                          {d[r][17]}
                        </Box>
                      </td>
                    </tr>
    ...
    if (c === 4) {
      return (
        <Tooltip
          ...
          enterDelay={1000}
        >
          <Box sx={{ color: getCellColor(d[r][c], 'tRes') }}>{d[r][c]}</Box>
        </Tooltip>
      )
    }
    if (c === 5) {
      return (
        <Tooltip
          ...
          enterDelay={1000}
        >
          <Box sx={{ color: getCellColor(d[r][c], 'wRes') }}>{d[r][c]}</Box>
        </Tooltip>
      )
    }
  ...
  const getColSpan = (r: number, c: number) =>
    r === 0 &&
    c === 5 &&
    props.setup.whole_resistance &&
    props.setup.local_resistance
      ? 2
      : undefined
  ...
  const getColWidth = (r: number, c: number) => {
    if (r === 0) {
      ...
      if (c === 4) return 170
      if (c === 5)
        return props.setup.whole_resistance && props.setup.local_resistance
          ? 80
          : 200
      if (c >= 6) return 200
    }
  ...
  return (
    ...
    >
      <table id="measTable">
        ...
        <tbody>
          {tableData.map((r, ii) => (
            <tr key={`dr${ii}`}>{r.map((c, jj) => getCell(ii, jj))}</tr>
          ))}
        </tbody>
      </table>
    </div>
