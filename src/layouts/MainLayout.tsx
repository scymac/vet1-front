...
// Types
import { PermissionType, ReportType, ScreenDim, Screens } from "types/types"
import { Measurement, Order, Setup } from "api/Interfaces"

// API
import { ApiAckPlcAlarms, ApiFinishOrder, ApiGetCurrentOrder, ApiGetPlcAlarms, ApiListMeasurementsByOrder, ApiListOrders, ApiListSetups, ApiStartOrder, dataExchangeRate } from "api/Requests"
...
export default function MainLayout() {
	...
	const getSetup = () => {
		const setupId = orderList.find((o) => o.id === selOrder)?.setup_id
		return setupList.find((s) => s.id === setupId)
	}

	const getScreen = () => {
		if (selectedScreen === "report") {
			return <ReportView measList={measList} order={orderList.find((o) => o.id === selOrder)!} setup={getSetup()!} responsible={responsible} setResponsible={(name: string) => setResponsible(name)} reportType={reportType} changeReportType={(type: ReportType) => setReportType(type)} />
		}
		if (selectedScreen === "alarm") {
			return <AlarmView screenDim={screenDimensions} currentAlarms={plcAlarms} />
		}
		if (selectedScreen === "setup") {
			return <SetupView screenDim={screenDimensions} setupList={setupList} permission={permission} listSetups={listSetups} />
		}
		return <MeasurementView screenDim={screenDimensions} orderStarted={orderStarted} selOrder={selOrder} setupList={setupList} orderList={orderList} measList={measList} tResUnit={tResUnit} sResUnit={sResUnit} setTResUnit={(val: boolean) => setTResUnit(val)} setSResUnit={(val: boolean) => setSResUnit(val)} setOrderStarted={(val: boolean) => setOrderStarted(val)} setSelOrder={(val: string) => setSelOrder(val)} listOrders={listOrders} openOrder={openOrder} finishOrder={finishOrder} />
	}
	...
	return (
		<>
			...
			/>
			<Container maxWidth={false} style={{ width: screenDimensions.width - left }} className={classes.mainContainer}>
				<MainHeader caption={getHeader()} top={top} left={left} serverOnline={serverOnline} plcOnline={plcOnline} quitAlarms={ackPlcAlarms} hasAlarms={plcAlarms.length > 0 && plcAlarms.filter((a) => a !== "1.00").length > 0} />
				<Container
					maxWidth={false}
					ref={ref}
					style={{
