...
// Styles
import { Measurement, Order, Setup, SResMeasurement } from "api/Interfaces"
...
export default function ReportView(props: Props) {
	...
	const [measMockList, setMeasMockList] = useState(() => {
		const l = []
		for (let i = 0; i < samples; i += 1) {
			l.push(defMeasurement)
		}
		return l
	})
	...
	const generatePdfDocument = async () => {
		...
		const pages = Array.from(printRef.current.children) as HTMLElement[]
		...
		for (let i = 0; i < pages.length; i++) {
			...
			page.style.border = "1px solid #fff" // make border white

			// Render to canvas
			const canvas = await html2canvas(page, { scale: 3, useCORS: true })
			const ratio = canvas.height / canvas.width
			const finalHeight = pdfWidthMM * ratio

			const imgData = canvas.toDataURL("image/png")

			if (i > 0) pdf.addPage() // add new page after the first
			pdf.addImage(imgData, "PNG", 0, 0, pdfWidthMM, finalHeight)

			// Restore original border
			page.style.border = originalBorder
		}

		pdf.save(`Messbericht_${props.order.order_no}.pdf`)
	}
	...
	const getReport = (variant: "document" | "snapshot") => {
		if (props.reportType === 1) {
			return (
				<Report1
					...
					measList={measMockList} //{measList.filter((m) => m.sample_no >= pageFrom && m.sample_no <= pageTo)}
					order={order}
					setup={setup}
					variant={variant}
				/>
		...
		if (props.reportType === 2) {
			return <Report2 responsible={props.responsible} measList={measList.filter((m) => m.sample_no >= pageFrom && m.sample_no <= pageTo)} order={order} setup={setup} variant={variant} />
		}
		return <Report3 responsible={props.responsible} measList={measList.filter((m) => m.sample_no >= pageFrom && m.sample_no <= pageTo)} order={order} setup={setup} variant={variant} />
	}
	...
	return (
		...
		>
			...
			>
				...
				<TextInputField value={props.responsible} fieldVariant="outlined" width={300} onChange={(val: string) => props.setResponsible(val)} />
				...
				>
					...
					<NumInputField value={pageFrom} onChange={(val: number) => setPageFrom(val > pageTo ? pageTo : val)} fieldVariant="outlined" height={30} width={80} minEqual maxEqual minValue={pageMin} maxValue={pageTo} precision={0} step={1} />
					...
					<NumInputField value={pageTo} onChange={(val: number) => setPageTo(val < pageFrom ? pageFrom : val)} fieldVariant="outlined" height={30} width={80} minEqual maxEqual minValue={pageFrom} maxValue={pageMax} precision={0} step={1} />
				</Box>
				<Button
					variant="contained"
					color="warning"
			...
			<div ref={printRef}>{getReport("snapshot")}</div>
		</div>
	)
}
