...
import { ApiEditOrder, ApiNewOrder, ApiStartOrder } from "api/Requests"
import toast, { Toaster } from "react-hot-toast"
import { timestampFormat2 } from "assets/functions/Conversions"

// Types
import { ScreenDim } from "types/types"
import { IdType, Measurement, NewOrder, NewSetup, Order, Setup } from "api/Interfaces"
...
export default function MeasurementView(props: Props) {
	...
	const getTableHeaders = () => {
		const s = getSetup()
		if (s !== null) {
			const list1 = ["Plattennr.", "Status", "Zeitstempel", "Dicke [mm]", `Durchgangswiderstand [kΩ${!props.tResUnit ? "" : ".cm"}]`, `Oberflächenwiderstand [kΩ${!props.sResUnit ? "" : " sq."}]`] // ["Plattennr.", "Status", "Zeitstempel"]
				...
				if (s.local_resistance) {
					...
					for (let i = 1; i < 13; i += 1) {
						const val = () => {
							switch (i) {
								case 1:
									return m.l_res1.resistance
								case 2:
									return m.l_res2.resistance
								case 3:
									return currentSetup?.is_half_plate ? null : m.l_res3.resistance
								case 4:
									return currentSetup?.is_half_plate ? null : m.l_res4.resistance
								case 5:
									return m.l_res5.resistance
								case 6:
									return m.l_res6.resistance
								case 7:
									return currentSetup?.is_half_plate ? null : m.l_res7.resistance
								case 8:
									return currentSetup?.is_half_plate ? null : m.l_res8.resistance
								case 9:
									return m.l_res9.resistance
								case 10:
									return m.l_res10.resistance
								case 11:
									return currentSetup?.is_half_plate ? null : m.l_res11.resistance
								case 12:
									return currentSetup?.is_half_plate ? null : m.l_res12.resistance
							}
						}
						d.push(t(val()))
					}
	...
	return (
		<>
			...
			/>
			<Box>
				<Box className={classes.orderButtonBox} style={{ width: props.screenDim.width - 220 }}>
					<IconButton
						icon="add"
					...
					<Box
						className={`${classes.menuButton} ${classes.noSelect} ${props.tResUnit ? classes.menuButtonSelected : classes.menuButtonUnselected}`}
						style={{
							marginLeft: "40px",
						}}
					...
					<Box className={`${classes.menuButton} ${classes.noSelect} ${props.sResUnit ? classes.menuButtonSelected : classes.menuButtonUnselected}`} onClick={() => props.setSResUnit(!props.sResUnit)}>
						{`Oberflächenwid. ${props.sResUnit ? "kΩ sq." : "kΩ"}`}
					</Box>
				</Box>

				<Box className={classes.orderBox} style={{ width: props.screenDim.width - 225 }}>
					{props.orderStarted ? (
						<>
							<Box className={classes.formItemText}>
								<Text text="Auftrag Nr :" type="p1" />
							</Box>
							<Box className={classes.formItemLabel1}>
								<Text text={currentOrder?.order_no!} type="h5" />
							</Box>
							<Box className={classes.formItemText}>
								<Text text="Produkt :" type="p1" />
							</Box>
							<Box className={classes.formItemLabel1}>
								<Text text={currentOrder?.product_no!} type="h5" />
							</Box>
							<Box className={classes.formItemText}>
								<Text text="Einstellung :" type="p1" />
							</Box>
							<Box className={classes.formItemLabel1}>
								<Text
							...
							<Box className={classes.formItemText}>
								<Text text="IST-Plattendicke :" type="p1" />
							</Box>
							<Box className={classes.formItemLabel1}>
								<Text text={Number(currentOrder?.thickness!).toFixed(3)} type="h5" />
							</Box>
							<IconButton
								icon="notes"
								tooltip={
						...
						<Box className={classes.formItemText4}>
							<Text text="kein Auftrag erstellt" type="h5" marginLeft={20} />
						</Box>
					)}
				</Box>
				{props.orderStarted ? (
					<Box className={classes.tableBox} style={{ width: props.screenDim.width - 225 }}>
						<MeasTable screenDim={props.screenDim} rawData={props.measList} headers={getTableHeaders()} data={getTableData()} setup={getConstants()} showTResOutOfSpec={props.tResUnit} showSResOutOfSpec={props.sResUnit} />
					</Box>
				) : null}
			</Box>
